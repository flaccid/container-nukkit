...
docker-build-upstream:: ## builds the docker image from upstream jar
		@echo http_proxy=$(HTTP_PROXY) http_proxy=$(HTTPS_PROXY)
		@echo building $(IMAGE_TAG)
		@docker build --pull \
			--build-arg=http_proxy=$(HTTP_PROXY) \
			--build-arg=https_proxy=$(HTTPS_PROXY) \
			-f Dockerfile.upstream-binary \
			-t $(IMAGE_TAG) $(WORKING_DIR)
...
get-latest-jar:: ## get the latest upstream jar
		@mkdir -p bin
	  @curl https://ci.opencollab.dev/job/NukkitX/job/Nukkit/job/master/lastSuccessfulBuild/artifact/target/nukkit-1.0-SNAPSHOT.jar > bin/nukkit-1.0-SNAPSHOT.jar

# the brancher used here would need to be modified from time-to-time
get-next-jar:: ## get the next upstream jar
		@mkdir -p bin
		@curl https://ci.opencollab.dev/job/NukkitX/job/Nukkit/job/1.16.100/lastSuccessfulBuild/artifact/target/nukkit-1.0-SNAPSHOT.jar > bin/nukkit-1.0-SNAPSHOT.jar

run:: ## run the jar
		@java -jar bin/nukkit-1.0-SNAPSHOT.jar
